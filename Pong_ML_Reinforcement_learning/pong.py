...
def stack_frames(frames_deque):
    return np.stack(frames_deque, axis=-1)
...
def load_progress(model):
    ...
    if os.path.exists(MODEL_FILE):
        try:
            ...
            print(f"Starting fresh: Weights on disk are incompatible with 4-frame input.")
...
# TEST MODE
def run_test(rom_name="pong"): # python3 pong.py --test
    print("\nRUNNING TEST MODE")
    ale = ALEInterface()
    ...
    ale.reset_game()
    frame_stack = deque([preprocess_frame(ale.getScreenRGB()) for _ in range(4)], maxlen=4)
    ...
    while not ale.game_over():
        ...
        frame_stack.append(preprocess_frame(ale.getScreenRGB()))

        display_img = cv2.resize(ale.getScreenRGB(), (400, 400))
            ...
            while not ale.game_over():
                if total_steps % 4 == 0:
                    if np.random.rand() <= epsilon:
                        ...
                        state_input = np.expand_dims(state, axis=0)
                        q_values = main_model(state_input, training=False)
                ...
                frame_stack.append(preprocess_frame(next_raw))
                next_state = stack_frames(frame_stack)
                ...
                if total_steps % 4 == 0 and len(memory) > BATCH_SIZE:
                    ...
                    states_b = np.array([m[0] for m in minibatch])
                    next_states_b = np.array([m[3] for m in minibatch])
